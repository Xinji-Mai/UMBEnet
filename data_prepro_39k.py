...
class data_prepro(Dataset):
    def __init__(self, 
        ...
        maskxlsx = pd.read_csv(os.path.join(self.dataset_dir,'EmoLabel/EmoLabel/train_All.csv'))
    ...
    def __getitem__(self, idx):
        ...
        f_path = os.path.join(self.f_image_dir,imgs_dir)

        if os.path.exists(f_path):
            f_frames, valid_list = self.get_all_video_frame(f_path)
        else:
            f_frames = torch.zeros(32,3,224,224)
            valid_list = torch.zeros(32)

        return f_frames, label, valid_list
