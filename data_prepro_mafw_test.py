...
class data_prepro_t(Dataset):
    def __init__(self, 
        ...
        self.items_to_load = {}
        maskxlsx = pd.read_csv(os.path.join(self.dataset_dir,'preprocess/test_random.csv'))
        ...
        for idx, row in meta.iterrows():
            ...
            label = row['single_label'] - 1
            # print(name,cap,label)
            self.meta.append([name,cap,label,row['mask']])
    ...
    def get_all_audio(self, audio_dir):
        ...
        #     # 处理wav为空的情况
        #     print(f"Warning: Audio data is empty for ident={ident}, idx={idx}")
        #     wav = torch.randn(1, 110250)  # 假设wav通常是1秒长，这需要根据你的情况进行调整

        

    def __getitem__(self, idx):
        imgs_dir, cap, label, mask = self.meta[idx]
